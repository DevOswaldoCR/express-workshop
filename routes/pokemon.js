...
pokemon.post("/", async (req, res, next) => {
    ...
    return res.status(500).json({code : 500, message : "Ocuerrio un error"});
    }
...
pokemon.put("/:id([0-9]{1,3})", async (req, res, next) => {
    ...
    if (pok_name && pok_height && pok_weight && pok_base_experience) {
        ...
        query += `pok_name='${pok_name}', `;
        query += `pok_height=${pok_height}, `;
        query += `pok_weight=${pok_weight}, `;
        // Nota: Esta última línea NO lleva coma al final porque sigue el WHERE
        query += `pok_base_experience=${pok_base_experience} `; 
